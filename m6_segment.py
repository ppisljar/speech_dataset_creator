...
def split_subsegments_on_internal_silence(subsegments: List[Segment], silences: List[Sil], tokens: List[Token]) -> List[Segment]:
    ...
    """
    MIN_INTERNAL_SILENCE_MS = 100  # Minimum silence duration to consider for splitting
    MIN_SUBSEGMENT_DURATION_MS = 400  # Minimum duration for each resulting subsegment
    MIN_TRANSCRIPTION_GAP_MS = 50  # Minimum gap in transcriptions to consider
    
    def split_single_subsegment(subseg: Segment) -> List[Segment]:
        """Recursively split a single subsegment on internal silences."""
        
        # Find silences that are within this subsegment (minimal margins)
        internal_silences = []
        for s_start, s_end, s_dur in silences:
            # Very minimal margins - just need to be inside the subsegment
            if (s_start > subseg.start_ms and s_end < subseg.end_ms and 
        ...
        # Get tokens for this subsegment - handle speaker mapping issues
        subseg_tokens = []
        for tok in tokens:
            # Handle speaker mapping: "1" -> "spk_00", "2" -> "spk_01", etc.
            tok_speaker = f"spk_{int(tok.speaker)-1:02d}" if tok.speaker.isdigit() else tok.speaker
            
            if (tok_speaker == subseg.speaker and 
                not (tok.end_ms <= subseg.start_ms or tok.start_ms >= subseg.end_ms)):
                subseg_tokens.append(tok)
        
        if not subseg_tokens:
            return [subseg]
        
        # Sort silences by duration (largest first)
        ...
        for silence in internal_silences:
            ...
            # Find tokens before and after this silence with generous overlap
            tokens_before = [tok for tok in subseg_tokens if tok.end_ms <= s_start + 300]
            tokens_after = [tok for tok in subseg_tokens if tok.start_ms >= s_end - 300]
            
            if tokens_before and tokens_after:
                # Check if we have enough content on both sides
                ...
                if transcription_gap >= MIN_TRANSCRIPTION_GAP_MS or s_dur >= 300:
                    ...
                    if (first_duration >= MIN_SUBSEGMENT_DURATION_MS and 
                        ...
                        # Assign tokens based on center point
                        first_tokens = []
                        second_tokens = []
...
def segment_audio(audio_path, json_path, outfile, silence_db: int = SILENCE_DB, min_silence_sec: float = MIN_SILENCE_SEC, min_seg_sec: float = MIN_SEG_SEC, max_seg_sec: float = MAX_SEG_SEC, silence_pad_ms: int = 50) -> None:
    ...
    for seg_idx, segdict in enumerate(segments, 1):
        ...
        raw_sub_segs = segdict.get('subs_merged', [])  # These are the raw subsegments before any processing
    ...
    print(f"[ok] Saved raw segments (before any processing) to {raw_outfile}")
