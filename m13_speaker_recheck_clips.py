...
def load_bad_segments(project_path):
        ...
        try:
            ...
            if speakers_data:
                for speaker_path, speaker_segments in speakers_data.items():
                    # Handle different formats of speaker_segments
                    if isinstance(speaker_segments, list):
                        # Each item could be a dict with 'filename' or just a string
                        for segment in speaker_segments:
                            if isinstance(segment, dict) and 'filename' in segment:
                                filename = segment['filename']
                                # Create the full path to this bad segment
                                full_bad_path = os.path.join(speaker_path, filename)
                                bad_segments_set.add(full_bad_path)
                    ...
                    elif isinstance(speaker_segments, dict) and 'processed_files' in speaker_segments:
                        # Old format with processed_files
                        for bad_file in speaker_segments['processed_files']:
                            full_bad_path = os.path.join(speaker_path, bad_file)
                            bad_segments_set.add(full_bad_path)
    ...
    return bad_segments_set


def find_audio_clips(project_path):
    """Find all audio clips in _segments folders."""
    clips = []
    segments_dirs = []
    ...
    for root, dirs, files in os.walk(project_path):
        for dir_name in dirs:
            if dir_name.endswith('_segments'):
                segments_dir = os.path.join(root, dir_name)
                segments_dirs.append(segments_dir)
    
    # Find all .wav files in speakers subdirectories
    for segments_dir in segments_dirs:
        speakers_dir = os.path.join(segments_dir, 'speakers')
                ...
                if os.path.isdir(speaker_path):
                    for file in os.listdir(speaker_path):
                        if file.endswith('.wav'):
                            clip_path = os.path.join(speaker_path, file)
...
def speaker_recheck(project_name, threshold=0.8, backends=None, max_workers=1):
    ...
    bad_segments = load_bad_segments(project_path)
    print(f"Loaded {len(bad_segments)} bad segments to skip")
    
    # Find all audio clips
    all_clips = find_audio_clips(project_path)
    print(f"Found {len(all_clips)} total audio clips")
    ...
    for clip in all_clips:
        relative_path = os.path.relpath(clip, project_path)
        clip_name = os.path.basename(clip)
        
        # Check if this clip is in the bad segments set
        # Bad segments contain full paths starting with "projects/project_name/"
        # We need to check the full clip path against bad segments
        is_bad = clip in bad_segments
        
        if not is_bad:
            good_clips.append(clip)
        else:
            skipped_count += 1
            # Uncomment for debugging individual skips
            # print(f"Skipping bad segment: {clip_name} -> {clip}")
    
    print(f"Skipped {skipped_count} bad segments")
    print(f"Processing {len(good_clips)} good clips")
    ...
    remaining_clips = [clip for clip in good_clips if clip not in processed_clips]
    ...
    def save_checkpoint(current_results, speaker_db, validation_output, speaker_db_path, project_name, threshold, backends, total_good_clips):
        ...
        try:
            ...
            print(f"Checkpoint saved: {len(current_results)}/{total_good_clips} clips processed ({len(current_results)/total_good_clips*100:.1f}%)")
            
        except Exception as e:
            print(f"Error saving checkpoint: {e}")
    
    total_good_clips = len(good_clips)  # Total clips we want to process (including already processed)
                ...
                if result is not None:  # Skip None results from shutdown
                    ...
                    if completed_count % 10 == 0:
                        save_checkpoint(results, speaker_db, validation_output, speaker_db_path, 
                                      project_name, threshold, backends, total_good_clips)
